...
const TeacherAssigns = () => {
  ...
  useEffect(() => {
    axiosinstance.get(`/get-all-courses`)
      .then(res => {
        const teacherCourses = res.data.data.filter(c => c.teacher_id?._id === id);
    ...
    <div className="left h-[80vh] w-[full] p-3 flex-col flex relative">
      <div className="">
        <div className="Assignments flex justify-between items-center">
          <h1 className="text-xl text-center font-qurova text-gray-700">Given Assignments</h1>
      ...
      {newToOld.length > 0 ? (
        newToOld.map((item) => (
          <div key={item._id} className="box my-1">
            <div className="hover:bg-[#f3f3f3] transition-all duration-300 rounded-md w-full flex items-center h-16">
              <div className="bg-green-500 w-12 h-10 m-1 rounded-full overflow-hidden flex justify-center items-center">
                <h1 className="text-white font-semibold text-2xl">{item.length}</h1>
              </div>
        ...
        <p className="text-sm text-gray-500 mt-4 text-center">No assignments found.</p>
      )}
    </div>
  );
};

export default TeacherAssigns;
