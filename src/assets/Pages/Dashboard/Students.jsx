...
const Students = () => {
  ...
  // console.log(users)
  const Students = users.filter((data) => data.role == 'Student')
  ...
  return (
    <div >
      ...
      {Students && <div className="cart flex flex-col items-cente justify-center mt-4 w-[300px] sm:w-full">
        <h1 className=' text-2xl font-qurova text-green-500 mx-3'>Students</h1>
      ...
      {/* <div className="rounded bg-red-500 md:w-[500px] w-[270px] h-[400px] z-40 top-[10px] left-[10px] absolute">
              </div> */}
      {/* {slcUser && (
        <div className="selected-user bg-white p-4 border rounded-2xl border-green-600 md:w-[500px] w-[270px] h-[500px] z-40 top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] absolute">
           <XMarkIcon className='size-5 absolute top-[10px] right-[10px]' onClick={() => { setslcUser(); setBackdrop(false) }}/>
          <h2 className="text-xl font-bold text-green-600">{slcUser.name}</h2>
          <div className="flex justify-center items-center">

          <div className="img size-[100px] overflow-hidden rounded-full flex ">
                        {slcUser.image ? <img className="object-cover object-center w-full h-full" src={`http://localhost:3031/User_dp/${slcUser.image}`} alt="" /> : <img className="object-cover object-center w-full h-full" src={`http://localhost:3031/User_dp/Default.jpg`} alt="" />}
                    </div>
          </div>
          <label htmlFor="Email"  className='text-green-600   font-qurova'>Email:</label>
          <div name={'Email'} className="email w-full px-4 py-3 h-12 my-1 rounded-md border bg-gray-100  border-green-500 font-semibold text-sm">{slcUser.email}</div>
          <label htmlFor="Role"  className='text-green-600   font-qurova'>Role:</label>
          <div name={'Role'} className="email w-full px-4 py-3 h-12 rounded-md border bg-gray-100  border-green-500 font-semibold text-sm">{slcUser.role}</div>
          <label htmlFor="Verified"  className='text-green-600   font-qurova'>Verified:</label>
          <div name={'Verified'} className="email w-full px-4 py-3 h-12 rounded-md border bg-gray-100  border-green-500 font-semibold text-sm">{slcUser.is_verified==false?'No':'Yes'}</div>
