...
const Fee_installments = () => {
    ...
    useEffect(() => {
        axiosinstance.get(`/student-installments_teacher/${id}`)
            ...
            {installments.length === 0 ? (
                ...
                installments.map((fee, i) => (
                    <div key={i} className="border border-green-500 rounded-lg p-4 mb-6 bg-white">
                        ...
                        <p className="text-sm text-gray-600">Student Name: <span className="font-medium capitalize">{fee.student_id.name}</span></p>
                        <p className="text-sm text-gray-600">Payment Method: <span className="font-medium capitalize">{fee.payment_method}</span></p>
                      {fee.total_fee==fee.paid_amount ? (<p className="text-sm text-gray-600">Total Fee Paid: <span className="font-medium">${fee.total_fee}</span></p>):(<div className="">
    ...
    {fee.payment_method === 'full' &&(
                     <p className="text-sm text-gray-600">Paid On: <span className="font-medium">{dayjs(fee.createdAt).format('DD MMM YYYY')}</span></p>) }
